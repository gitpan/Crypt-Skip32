...
# Number of bytes in the 10 byte key.
sub keysize {
  return 10;
}
...
sub encrypt {
  ...
  die "plaintext must be 4 bytes long"
...
sub decrypt {
  ...
  die "ciphertext must be 4 bytes long"
    unless scalar @input_bytes == 4;
...
$key is a key of C<keysize> (10) bytes.
...
Returns the size (in bytes) of the key.  This is always 10 bytes.
...
security.  Reviewers and testers welcomed.

=head1 TODO

This version of Skip32 is implemented entirely in Perl.  Since there
is a free C implementation of the algorithm, this should probably be
made available with XS or Inline::C so that it can run a bit faster.
...
The C version of Skip32 by Greg Rose (see below) is explicitly "not
copyright, no rights reserved".  Even so, permission was requested and
granted to make a Perl version available on the CPAN.
